 ...
 * @returns
 */
export var getPrice = function (options) {
    ...
    var origin = !isUndefined(originTotal) ? originTotal : total;
    // 最小原价 / 最大原价
    var _d = __read(isArray(origin) ? origin : [origin, origin], 2), minOrigin = _d[0], maxOrigin = _d[1];
    // 总价计算
    if (isArray(total)) {
        // 当价格为一个区间范围时
        // 最小价格 / 最大价格
        var _e = __read(total, 2), minTotal = _e[0], maxTotal = _e[1];
        // 最小差价 / 最大差价
        var _f = __read([minOrigin - minTotal, maxOrigin - maxTotal], 2), minDiff = _f[0], maxDiff = _f[1];
