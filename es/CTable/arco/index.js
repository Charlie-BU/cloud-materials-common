...
import { TableRow, TableRowMemo, TableCell, TableCellMemo, TableHeaderCell, Toolbar, LoadMoreTableBodyWrapper, } from './components';
...
import { useInnerCreateTable, useAutoUpdateTableWidth, usePolling, useControlledConfig, useMergedTableConfig, } from './hooks';
import { merge, getTableConfig, defineColumn, defineColumns, controlPagination, changeToolbarValues, defineToolbar, getArcoPropsFilterPagination, createPolling, formatUrlQueryConfig, changeToolbarValuesPure, } from './utils';
...
var renderTable = function (props) {
    return React.createElement(ArcoTable, __assign({}, props));
};
export var InnerTable = observer(function (_a) {
    var table = _a.table, className = _a.className, style = _a.style;
    var ctx = useCConfigContext();
    ...
    var tableConfig = mapTableConfig(table, ctx);
    var extraConfig = table.config.extraConfig;
    // 保证传给 arco 的 components 配置的不变性
    // 否则 arco 每次都会卸载 row、cell，再重新渲染
    var tableCustomComponents = useMemo(function () { return ({
        body: {
            row: (extraConfig === null || extraConfig === void 0 ? void 0 : extraConfig.useMemoTableRow) ? TableRowMemo : TableRow,
            cell: (extraConfig === null || extraConfig === void 0 ? void 0 : extraConfig.useMemoTableCell) ? TableCellMemo : TableCell,
    ...
    var originalArcoProps = __assign(__assign(__assign({}, tableConfig), { components: tableCustomComponents }), getDefaultTableProps(table));
    var mergedArcoProps = merge(originalArcoProps, isFn(table.config.arcoTableProps)
    ...
    var arcoProps = getArcoPropsFilterPagination(table, tableConfig, mergedArcoProps);
    // 2. 如果是自定义渲染分页，controlPagination 会返回分页的 ReactNode
    var customPaginationNode = controlPagination(table, mergedArcoProps, classScope, arcoPrefixCls);
    useAutoUpdateTableWidth(table, tableContainerRef);
    return (React.createElement(TableProvider, { table: table },
        React.createElement("div", { className: cls(
            ...
            table.toolbar ? React.createElement(Toolbar, null) : null,
            React.createElement(Spin, { icon: React.createElement(IconLoading, null), loading: table.loading, style: { width: '100%' }, className: "".concat(classScope, "-spin"), size: 48 },
                React.createElement("div", { className: "".concat(classScope, "-container"), ref: tableContainerRef },
...
export var _Table = function (props, ref) {
    var mergedTableConfig = useMergedTableConfig(props.config);
    // 配置了 url query，需要处理 config
    var propsConfig = formatUrlQueryConfig(mergedTableConfig);
    var table = useInnerCreateTable(propsConfig, props.table);
    // 标记 table 是否卸载
    ...
    useImperativeHandle(ref, function () { return table; });
    // 处理轮询
    var polling = usePolling({ table: table });
    useEffect(function () {
        var timer;
    ...
    return React.createElement(InnerTable, { table: table, className: props.className, style: props.style, forceUpdateFlag: table.config });
};
export var Table = Object.assign(forwardRef(_Table), {
    getConfig: getTableConfig,
    defineConfig: getTableConfig,
    defineColumn: defineColumn,
    defineColumns: defineColumns,
    defineToolbar: defineToolbar,
    getTextWidth: getTextWidth,
    changeToolbarValues: changeToolbarValues,
    changeToolbarValuesPure: changeToolbarValuesPure,
