 ...
 * @returns 父节点 field
 */
export var getParentField = function (field) {
 ...
 * @returns 兄弟节点
 */
export var getBrotherField = function (currentField, brotherPath) {
    var _a, _b, _c, _d, _e;
    var parentField = getParentField(currentField);
...
export var checkArrayDuplicates = function (params) {
    ...
    var _b = getGlobalContextConfig(), locale = _b.locale, formatLocale = _b.formatLocale;
    onFieldReact(arrayFieldName, function (field, form) {
        var _a, _b;
        ...
        if (!field.value || ((_a = field === null || field === void 0 ? void 0 : field.value) === null || _a === void 0 ? void 0 : _a.length) <= 0)
            return;
        var subNames = isArray(itemName) ? itemName : itemName ? [itemName] : [];
        var subMessage = isArray(message) ? message : message ? [message] : [];
        // 数组值为对象，这时为对象数组
        var isObjectArray = isObject(first(field.value));
        var valuesMap = {};
        if (isObjectArray) {
            ...
            untracked(function () {
                duplicateTag.forEach(function (duplicate, index) {
                    ...
                    query.forEach(function (item) {
                        if (FormPath.parse("".concat(arrayFieldName, ".").concat(index).concat(isObjectArray ? '.*' : '')).match(item.path.toString())) {
                            if (isDataField(item)) {
