 ...
 */
export function formatInitValue(initValue) {
    if (!initValue)
        return undefined;
    return initValue.map(function (v) { return pick(v, 'value').value; });
}
/**
 * 生成完整初始值的控制参数
 * @param initValue 传入组件的初始值
 ...
 */
export function getInitItemControl(initValue, itemsKey) {
    if (!initValue)
        return [];
    return initValue.map(function (item) { return (__assign({ disableDelete: false, disableDeleteTip: '', rowDisabled: false, rowDisabledTip: '', 
        /** 整合初始有disableChange的item和没有disableChange的item的disableChange */
        disableChange: __assign({}, getInitDisableChange(itemsKey, isBoolean(item.rowDisabled) ? item.rowDisabled : false, item.disableChange)) }, omit(item, 'value', 'disableChange'))); });
}
/**
 ...
 * */
export function getInitDisableChange(itemsKey, rowDisable, initDisableChange) {
    var e_1, _a;
    var _b, _c, _d;
    var disableChange = {};
    try {
        for (var itemsKey_1 = __values(itemsKey), itemsKey_1_1 = itemsKey_1.next(); !itemsKey_1_1.done; itemsKey_1_1 = itemsKey_1.next()) {
...
export function getEmptyItemValues(items) {
    var e_2, _a;
    var val = {};
    try {
 ...
 */
export function changeItemControl(old, editingDisableVerify, itemsKey) {
    var e_3, _a;
    ...
    if (disableItemsIndex) {
        try {
            for (var disableItemsIndex_1 = __values(disableItemsIndex), disableItemsIndex_1_1 = disableItemsIndex_1.next(); !disableItemsIndex_1_1.done; disableItemsIndex_1_1 = disableItemsIndex_1.next()) {
                ...
                if (typeof item === 'number' && item < newControl.length) {
                    ...
                    if (index < newControl.length) {
                        ...
                        newControl[index].disableChange = __assign(__assign({}, newControl[index].disableChange), disableChange);
                    }
                }
            }
        }
 ...
 */
export function repeatLabelValidator(v, cb, needCheck, itemValues, message) {
    var e_4, _a;
    if (!v || !itemValues || !needCheck)
        return;
    try {
        for (var itemValues_1 = __values(itemValues), itemValues_1_1 = itemValues_1.next(); !itemValues_1_1.done; itemValues_1_1 = itemValues_1.next()) {
            var value = itemValues_1_1.value;
            if (Array.isArray(value) && Array.isArray(v)) {
                if (isEqual(value, v) && value.length && v.length)
                    cb(message);
            }
            else {
                if (value === v)
...
export function needCheck(repeatValidator, label) {
 ...
 */
export function getItemValuesWidthOutIndex(listValues, label, index) {
 ...
 *
 */
export function disabledItemPopover(usePopoverVerify, disableRow, content) {
 ...
 *
 */
export function getAddDefaultValue(items) {
    var defaultValue = {};
