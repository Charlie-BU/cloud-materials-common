...
export interface CListEditorRules extends Omit<CPopoverVerifyRule, 'validateLevel' | 'validator'> {
    validator?: (value: any, callback: (error?: ReactNode) => void, // callback放这里，兼容已经使用的用户和CPopoverVerifyRule的validator
    arr: Record<string, any>, all: Record<string, any>[], idx: number) => void;
}
/**
 * @title Item
...
export interface EditingDisableVerify {
    ...
    disableItemsIndex?: (number | (Omit<InitValue, 'value'> & {
        index: number;
        [key: string]: any;
    }))[];
    /** item不可删除、修改的提示 */
...
export type ComponentProps = Omit<ComponentWrapperProps, 'index' | 'component' | 'changeListValue' | 'handleEditingDisableVerify' | 'label'>;
/** addBtn部分需要参数*/
