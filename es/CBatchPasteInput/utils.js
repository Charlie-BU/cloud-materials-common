...
// 根据预设IP校验模式校验输入字符串
export var isLegalIp = function (value, mode) {
    var result = false;
    switch (mode) {
...
// 将字符串按照分隔符进行切割，返回数组
export var formatStr2Arr = function (str, separator) {
    var e_1, _a;
    ...
    try {
        for (var finalSeparator_1 = __values(finalSeparator), finalSeparator_1_1 = finalSeparator_1.next(); !finalSeparator_1_1.done; finalSeparator_1_1 = finalSeparator_1.next()) {
...
// 判断一个字符串中是否包含分隔符
export function hasSeparatorInStr(str, separator) {
    var finalSeparator = combineSeparator(separator);
