...
export var findItem = function (data, key) {
    return data.find(function (item) { return item.key === key; });
};
export var findItemByPath = function (pathParentKeys, transferTree) {
    var item = findItem(transferTree, pathParentKeys[0]);
...
export var formatAddItem = function (item) {
    var _a, _b;
    var childrenData = (_a = item.children) === null || _a === void 0 ? void 0 : _a.map(formatAddItem);
    var isItemCheckAll = item.isClickCheckAll || !((_b = item.children) === null || _b === void 0 ? void 0 : _b.length);
    return __assign(__assign({}, item), { isLeaf: isItemCheckAll, children: isItemCheckAll ? [] : childrenData, 
        // 选中时的实际子数据。当全选节点在显示上其为叶子节点，但外层可能有使用此时子数据的需求，因此另外维护该数据
        childrenData: childrenData });
};
// 生成与已选中列表 merge 的数据。其需要有完整的层级结构
export var generateMergeItem = function (data, pathParentKeys, addItem) {
    ...
    if (mergeItem) {
        return __assign(__assign({}, mergeItem), { children: [generateMergeItem(mergeItem.children || [], pathParentKeys.slice(1), addItem)] });
    }
    return addItem;
};
export var mergeAddItem = function (newItem, targetData) {
    var _a;
    ...
    (_a = newItem.children) === null || _a === void 0 ? void 0 : _a.forEach(function (child) {
        ...
        var data = mergeAddItem(child, targetNode.children || []);
...
export var getNodeAllChild = function (node) {
    var _a, _b;
    ...
    (_b = node.children) === null || _b === void 0 ? void 0 : _b.forEach(function (child) {
        childIds = childIds === null || childIds === void 0 ? void 0 : childIds.concat(getNodeAllChild(child));
    });
    return childIds;
};
export var getSourceCheckedKeys = function (sourceData, targetFlattenMap) {
    var checkedKeys = [];
    var halfCheckedKeys = [];
    sourceData.forEach(function (item) {
        ...
        if (targetItem === null || targetItem === void 0 ? void 0 : targetItem.isLeaf) {
            ...
            var childIds = getNodeAllChild(item);
            checkedKeys.push.apply(checkedKeys, __spreadArray([], __read(childIds), false));
        }
        else {
            // 可展开时，说明选中了该节点的子节点，该节点为半选状态
            halfCheckedKeys.push(item.key);
            var _a = getSourceCheckedKeys(item.children || [], targetFlattenMap), childCheckedKeys = _a.checkedKeys, childHalfCheckedKeys = _a.halfCheckedKeys;
            checkedKeys = __spreadArray(__spreadArray([], __read(checkedKeys), false), __read(childCheckedKeys), false);
            halfCheckedKeys.push.apply(halfCheckedKeys, __spreadArray([], __read(childHalfCheckedKeys), false));
        }
    });
    return { checkedKeys: checkedKeys, halfCheckedKeys: halfCheckedKeys };
};
export var deleteRemoveItem = function (_a) {
    var _b, _c;
    ...
    var cloneTargetTree = cloneDeep(targetTree);
    ...
    if ((targetItem === null || targetItem === void 0 ? void 0 : targetItem.isLeaf) && sourceItem && !(sourceItem === null || sourceItem === void 0 ? void 0 : sourceItem.isLeaf)) {
        ...
        targetItem.children = (_b = sourceItem.children) === null || _b === void 0 ? void 0 : _b.map(formatAddItem);
    }
