...
var refFetchId = _Date$now();
var debouncedFetchUser = debounce(function (_, callback) {
  refFetchId = _Date$now();
  ...
  fetch('https://randomuser.me/api/?results=5').then(function (response) {
    ...
    if (refFetchId === fetchId) {
      var _context;
      var options = _mapInstanceProperty(_context = body.results).call(_context, function (user) {
        var _context2;
        return {
          ...
          })), _concatInstanceProperty(_context2 = "".concat(user.name.first, " ")).call(_context2, user.name.last)),
          value: user.email
        };
      });
      callback(options);
...
var AsyncDataSourceEffect = function AsyncDataSourceEffect(pattern, service) {
  var keyword = observable.ref('');
  onFieldInit(pattern, function (field) {
