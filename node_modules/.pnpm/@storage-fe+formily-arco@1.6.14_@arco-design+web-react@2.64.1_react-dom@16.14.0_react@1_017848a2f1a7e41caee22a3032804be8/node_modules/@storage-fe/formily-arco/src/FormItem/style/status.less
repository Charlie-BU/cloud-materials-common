// Copied from Select
@import (reference) "@arco-design/web-react/es/style/index.less";
@import (reference) "@arco-design/web-react/es/Form/style/token.less";

@arco-formily-item: ~"@{prefix}-formily-item";
...
.validate-status (@status) {
  ...
  & {
    ...
    .@{prefix}-textarea:not(.@{prefix}-textarea-disabled) {
      .@{arco-formily-item}-status-@{status} & {
        border-color: ~"@{form-color-border_@{status}}";
        background-color: ~"@{form-color-bg_@{status}}";
    ...
    .@{prefix}-textarea {
      .@{arco-formily-item}-status-@{status} &&-focus,
      .@{arco-formily-item}-status-@{status} &&-focus:hover {
        border-color: ~"@{form-color-border_@{status}_focus}";
        background-color: ~"@{form-color-bg_@{status}_focus}";
        box-shadow: 0 0 0 ~"@{form-size-shadow_@{status}_focus}" ~"@{form-color-shadow_@{status}_focus}";
      }
    ...
    .@{arco-formily-item}-status-@{status} .@{prefix}-input-inner-wrapper .@{prefix}-input {
      background: none;
      box-shadow: none;
  ...
  .@{arco-formily-item}-message-status-@{status} {
    .@{arco-formily-item}-message-help {
      color: ~"@{form-color-tip-text_@{status}}";

      .@{arco-formily-item}-message-help-warning {
...
.@{arco-formily-item}-has-feedback {
  .select-view(@prefix) {
