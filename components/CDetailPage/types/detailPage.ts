...
export interface ICDetailPageOptions<DetailData extends ObjRecord, GlobalScopeType extends ObjRecord = any> {
   ...
   */
  unmountContentWhenRefresh?: (options: { detailPage: DetailPage<DetailData, GlobalScopeType> }) => boolean;
  /**
   * @zh 是否需要全局loading
   */
  needLoading?:
    | boolean
    | ((options: { data: DetailData; detailPage: DetailPage<DetailData, GlobalScopeType> }) => boolean);
  /**
  ...
  globalErrorConfig?:
    | IBaseErrorConfig
    | ((options: { data: DetailData; detailPage: DetailPage<DetailData, GlobalScopeType> }) => IBaseErrorConfig);
  /**
   *
  ...
  tabErrorConfig?:
    | IBaseErrorConfig
    | ((option: { data: DetailData; detailPage: DetailPage<DetailData, GlobalScopeType> }) => IBaseErrorConfig);
  /**
  ...
  arcoTabProps?:
    | TabsProps
    | ((options: { data: DetailData; detailPage: DetailPage<DetailData, GlobalScopeType> }) => TabsProps);
  ...
  cDetailPageHeaderProps?:
    ...
    | ((options: { data: DetailData; detailPage: DetailPage<DetailData, GlobalScopeType> }) => ICDetailPageHeaderProps);
  ...
  cDetailPageContentProps?:
    ...
    | ((options: { data: DetailData; detailPage: DetailPage<DetailData, GlobalScopeType> }) => IDetailPageContentProps);
  /**
   * @zh 额外配置
   */
  extraConfig?: {
...
// 这里将DetailPage的实际类型和实际使用的组件类型分离开来，不使其强绑定使用的组件
...
export type DefineDetailPageConfig = <DetailData extends ObjRecord, GlobalScopeType extends ObjRecord = ObjRecord>(
  config: ICDetailPageOptions<DetailData, GlobalScopeType>,
) => ICDetailPageOptions<DetailData, GlobalScopeType>;

export type IDetailPageComponent = React.ForwardRefExoticComponent<
  ICDetailPageProps & RefAttributes<DetailPage<any>>
> & {
...
export type InnerRefreshOptions = IRefreshOptions & { isFirstRender?: boolean };
