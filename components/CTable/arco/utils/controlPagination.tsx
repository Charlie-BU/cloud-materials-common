...
import { StatusChangeReason } from '../../core';
import { BottomLeftToolbar } from '../components';

/**
 * 规范：当数据总数小于某个数值时，隐藏分页
 * @param table
 */
export const hidePaginationWhenTotalLessThan = (table: TableModel<any>) => {
  ...
  return table.total <= (table.config.extraConfig?.hidePaginationTotal || 10) && table.isOnFirstPage();
};

// 是否应该隐藏 table 底部的操作按钮
export const shouldHideBottomButton = (table: TableModel<any>) => {
  // 左下角的操作按钮隐藏逻辑同下面注释的分页
  return (
    // 初始化时隐藏分页，避免先有分页，后又隐藏导致分页按钮闪现
    (table.status.statusChangeReason === StatusChangeReason.INIT && table.loading) ||
    // 没有数据时，出现错误，才能隐藏分页
...
export const getArcoPropsFilterPagination = (
  table: TableModel<any>,
  // mapTableConfig 返回的 arcoProps
  tableConfig: ArcoTableProps,
  // merge 之后的 arcoProps
  mergedArcoProps: ArcoTableProps,
) => {
  /**
  ...
  if (table.config.extraConfig?.useArcoPagination === true) {
    return { ...mergedArcoProps };
  }

  // 渲染自定义分页器时，把 arco table 的 pagination 干掉
  return { ...mergedArcoProps, pagination: false };
};
...
export const controlPagination = (
  ...
  const { pagination, renderPagination } = mergedArcoProps;
  // 原始的分页配置
  const paginationNode = pagination && <Pagination {...(pagination as PaginationProps)} />;
  // 这里直接复用了 arco 的分页容器样式，好处是可以直接应用全局的风格样式，坏处是可能会受 arco 样式的影响
  const containerClassName = `${tablePrefixCls}-pagination ${arcoPrefixCls}-table-pagination`;
  // 兼容业务自定义渲染分页器的情况，如果是业务自渲染分页，暂时不支持渲染 BottomLeftToolbar，如果业务方有诉求，可以自己渲染
  if (renderPagination) {
    return <div className={containerClassName}>{renderPagination(paginationNode)}</div>;
  }

  const showPagination = Boolean(pagination);
  const showBottomLeft = Boolean(table.config.toolbar?.bottomLeft) && !shouldHideBottomButton(table);
  if (showPagination || showBottomLeft) {
    return (
      <div
        className={cls(
          containerClassName,
          showBottomLeft && `${tablePrefixCls}-pagination-with-toolbar`,
          showBottomLeft && table.selectedRowKeys.length > 0 && `${tablePrefixCls}-pagination-with-toolbar-highlight`,
        )}
      >
        {showBottomLeft && <BottomLeftToolbar config={table.config.toolbar!} />}
