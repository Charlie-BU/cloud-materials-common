 ...
 * @title 远程拉取函数返回数据类型
 */
export type CFormAsyncSelectFetchDataSource = CFormAsyncSelectDataSource & ({ total: number } | { noMore: boolean });
/**
...
export interface CFormAsyncSelectProps extends Omit<SelectProps, 'options' | 'dropdownRender'> {
  ...
  dropdownRender?: (
    menu?: ReactNode,
    originalMenu?: ReactNode,
    loadingType?: UseCFormAsyncSelectState['loadingType'],
 ...
 */
export type UseCFormAsyncSelectProps = Pick<
 ...
 * @description 状态控制
 */
export interface UseCFormAsyncSelectControl
  extends Pick<ReturnType<typeof useInfiniteScroll>, 'cancel' | 'loadMore' | 'mutate'> {
  reload: () => void;
