...
export interface CDrawerSelectProps<T extends R = any> extends Omit<CDrawerTableSelectProps<T>, 'type'> {
  ...
  /** 自定义渲染基础组件 */
  customRender?: ({ close, open }: { close: () => void; open: () => void }) => ReactNode;
  /**
...
// /**
//  * @title CDrawerTableSelectProps
//  */
export interface CDrawerTableSelectProps<T extends R = any>
  extends CDrawerProps,
    Pick<
      CTableSelectProps,
      'type' | 'columns' | 'fetcher' | 'data' | 'rowKey' | 'value' | 'onChange' | 'afterSelectRow'
    > {
  /**
   * @default false
