/*
 * @Author: youjingyu
 * @Date: 2021-10-13 13:54:49
 * @LastEditTime: 2021-12-14 20:52:30
 * @LastEditors: youjingyu
 * @Description:
 */
import type {
  TableColumnProps as ArcoTableColumnProps,
  TableProps as ArcoTableProps,
  PaginationProps,
} from '@arco-design/web-react';
import type { CSSProperties, FC, ReactNode } from 'react';
import type {
  CellComponentRenderProps,
  Column as ColumnCoreModel,
  AutoWidth as CoreAutoWidth,
  ColumnConfig as CoreColumnConfig,
  Filter as CoreFilter,
  Formatter as CoreFormatter,
  Resize as CoreResize,
  // Toolbar,
  RowSelection as CoreRowSelection,
  Sorter as CoreSorter,
  Summary as CoreSummary,
  TableConfig as CoreTableConfig,
  FilterDropdownRenderProps,
  Row,
  Table as TableCoreModel,
  ToolbarItemRenderProps,
} from '../../core';
...
export interface TableModel<DataItemType extends R, GlobalScopeType extends R = any>
  extends TableCoreModel<DataItemType, GlobalScopeType> {
  config: TableConfig<DataItemType, GlobalScopeType>;
  // toolbar: Toolbar<FormInstance>;
}

export interface ColumnModel<DataItemType extends R> extends ColumnCoreModel<DataItemType> {
  config: ColumnConfig<DataItemType> & { dataIndex: string };
}

type RowSectionTypes = 'radio' | 'checkbox';
interface RowSection<DataItemType extends R>
  // 暂时不对外暴露 component、decorator，等有明确需求的时候再说
  extends Omit<CoreRowSelection<DataItemType>, 'component' | 'componentProps' | 'decorator' | 'decoratorProps'> {
  type?: RowSectionTypes;
}

type FilterTypes = 'select' | 'range' | 'searchInput';
interface Filter<DataItemType extends R, GlobalScopeType extends R = any>
  extends CoreFilter<DataItemType, GlobalScopeType> {
  type?: FilterTypes;
  component?: FC<
    FilterDropdownRenderProps<DataItemType, GlobalScopeType> & {
...
interface Formatter<DataItemType extends R, GlobalScopeType extends R = any>
  extends CoreFormatter<DataItemType, GlobalScopeType> {
  type?: FormatterTypes;
}
...
interface Summary<DataItemType extends R, GlobalScopeType extends R = any>
  extends CoreSummary<DataItemType, GlobalScopeType> {
  type?: SummaryTypes;
}
type AutoWidthTypes = 'default';

interface AutoWidth<DataItemType extends R, GlobalScopeType extends R = any>
  extends CoreAutoWidth<DataItemType, GlobalScopeType> {
  type?: AutoWidthTypes;
}
...
interface BaseColumnConfig<DataItemType extends R, GlobalScopeType extends R = any>
  extends CoreColumnConfig<DataItemType> {
  title: ReactNode;
  tooltip?: ReactNode;
  /**
  ...
  //  */
  // libColumnComponentProps?: Partial<ArcoTableColumnProps>;
  arcoColumnProps?:
  ...
  formatter?:
    | FormatterTypes
    | Formatter<DataItemType, GlobalScopeType>
    | Formatter<DataItemType, GlobalScopeType>['formatterFn'];
...
export type ColumnConfig<DataItemType extends R, GlobalScopeType extends R = any> = Omit<
  BaseColumnConfig<DataItemType, GlobalScopeType>,
  'component' | 'componentProps'
> &
  CellComponentConfig<DataItemType, GlobalScopeType>;
...
export interface TableConfig<DataItemType extends R, GlobalScopeType extends R = any>
  extends CoreTableConfig<DataItemType, GlobalScopeType> {
  ...
  // /**
  //  * @deprecated Use arcoTableProps instead.
  //  */
  // libTableComponentProps?:
  //   | Partial<ArcoTableProps>
  //   | ((table: TableModel<DataItemType>, originalProps: ArcoTableProps) => Partial<ArcoTableProps>);
  arcoTableProps?:
    | Partial<ArcoTableProps>
    | ((table: TableModel<DataItemType, GlobalScopeType>, originalProps: ArcoTableProps) => Partial<ArcoTableProps>);
   ...
   * 2. 完全自定义的 ReactNode
   */
  noDataElement?: (options: { table: TableModel<DataItemType, GlobalScopeType> }) => CResultProps | ReactNode;
  /**
...
type ToolbarComponentProps<DataItemType extends R, T> = T | ((options: ToolbarItemRenderProps<DataItemType>) => T);
type CellComponentProps<DataItemType extends R, T> = T | ((options: CellComponentRenderProps<DataItemType>) => T);

// eslint-disable-next-line @typescript-eslint/no-unused-vars
type AllComponentsMap<DataItemType extends R> = {
  RefreshBtn: ToolbarComponentProps<DataItemType, RefreshBtnProps>;
  ColConfigBtn: ToolbarComponentProps<DataItemType, ColConfigBtnProps>;
  ExportDataBtn: ToolbarComponentProps<DataItemType, ExportDataBtnProps>;
  FunctionBtnList: ToolbarComponentProps<DataItemType, FunctionBtnListProps>;
  CToolbarOperationMenu: ToolbarComponentProps<DataItemType, COperationMenuProps>;
  CSearch: ToolbarComponentProps<DataItemType, CSearchProps>;
  CSimpleSearch: ToolbarComponentProps<DataItemType, CSimpleSearchProps>;
  CCascaderSearch: ToolbarComponentProps<DataItemType, CCascaderSearchProps>;
  CCombineSearch: ToolbarComponentProps<DataItemType, CCombineSearchProps>;

  Number: CellComponentProps<DataItemType, NumberProps>;
  // Tag: CellComponentProps<DataItemType, TagProps>;
  // 内置云计算其他物料组件
  CTag: CellComponentProps<DataItemType, CTagProps>;
  CStatus: CellComponentProps<DataItemType, CStatusProps>;
  CNameInfo: CellComponentProps<DataItemType, CNameInfoProps>;
  COperationMenu: CellComponentProps<DataItemType, COperationMenuProps>;
  CPopupEdit: CellComponentProps<DataItemType, CPopupEditProps>;
  CInlineEdit: CellComponentProps<DataItemType, CInlineEditProps>;
  CCollapse: CellComponentProps<DataItemType, CCollapseProps<any>>;
};
// eslint-disable-next-line @typescript-eslint/no-unused-vars
type ToolbarComponentName =
  | 'RefreshBtn'
  | 'ColConfigBtn'
  | 'ExportDataBtn'
  | 'CSearch'
