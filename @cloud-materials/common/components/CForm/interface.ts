...
export type CFormStep = Omit<IFormStep, 'form'> & { form: CForm };
 ...
 */
// ❓为什么要显式定义类型？
// 显式定义类型可以让引用了这个变量的地方生成d.ts时不用内联它们而是通过import("../CForm/interface").DefaultBuiltInComponentMapType这种形式
// 否则如bit这样的库在生成类型时会自动生成内联的类型，造成不同版本的物料出现类型错误
...
export interface DefineField<C extends PureComponentsMap = DefaultBuiltInComponentMapType> {
  ...
  <
    ...
    Fields extends Partial<CFieldConfig<F, D, C>[]>,
    LastField extends Partial<CFieldConfig<F, D, C>>,
  >(
    field: CFieldConfig,
    // eslint-disable-next-line @typescript-eslint/ban-types
    ...restField: [...Fields, (keyof CFieldConfig | (string & {}))[] | LastField]
  ): UnionToIntersection<Fields[number] | LastField>;
}

export interface DefinePartialField<C extends PureComponentsMap = DefaultBuiltInComponentMapType> {
  <
    F extends ObjectType = ObjectType,
    D extends ObjectType = ObjectType,
    Field extends Partial<CFieldConfig<F, D, C>> = Partial<CFieldConfig<F, D, C>>,
  >(
    field: Field,
  ): Partial<Field extends { type: 'VoidField' } ? VoidFieldType : BasicFieldType>;
}
...
export interface FormType<
  C extends PureComponentsMap = DefaultBuiltInComponentMapType,
  DecoratorType extends React.JSXElementConstructor<FormStepDecoratorProps> = React.JSXElementConstructor<DefaultCFormDecoratorProps>,
> {
  /**
   * @zh 组件构造器定义
   */
  <FormData extends ObjectType = any, GlobalData extends ObjectType = any>(
    props: CFormProps<FormData, GlobalData, C, DecoratorType>,
  ): React.ReactElement | null;

  /**
   * 对应到 CField 组件
   */
  Field: <FormData extends ObjectType = any, GlobalData extends ObjectType = any>(
    props: CFieldConfig<FormData, GlobalData, C>,
  ) => React.ReactElement | null;
   ...
   */
  registerConfig: <M extends PureComponentsMap>(
    componentsMap: M,
    options?: CFormRegisterConfigOptions<Omit<C, keyof M> & M>,
  ) => FormType<Omit<C, keyof M> & M>;
   ...
   */
  defineComponentOption: <
    Component extends keyof C | React.JSXElementConstructor<any>,
    P extends Component extends keyof C
      ? C[Component]
  ...
  >(
    component: Component,
    props?: React.ComponentProps<P> | ((form: CForm, parentField: CGeneralField) => React.ComponentProps<P>),
  ) => { component: Component; props?: typeof props };
   ...
   * @zh 声明多个字段的配置。
   */
  defineFields: <F extends ObjectType = ObjectType, D extends ObjectType = ObjectType>(
    fields: CFieldConfig<F, D, C>[],
  ) => typeof fields;

  /**
   * @zh 声明一个普通表单的配置
   */
  defineConfig: <F extends ObjectType = ObjectType, D extends ObjectType = ObjectType>(
    config: CFormConfig<F, D, C>,
  ) => typeof config;

  /**
   * @zh 声明一个步骤表单的配置
   * */
  defineStepConfig: <F extends ObjectType = ObjectType, D extends ObjectType = ObjectType>(
    config: CFormStepConfig<F, D, C>,
  ) => typeof config;
...
export interface CFormProps<
  ...
  ComponentMap extends PureComponentsMap = DefaultBuiltInComponentMapType,
  DecoratorType extends React.JSXElementConstructor<FormStepDecoratorProps> = React.JSXElementConstructor<FormStepDecoratorProps>,
> {
  /**
   * @zh 外部传入的 form
...
export type DefaultCFormProps<T extends ObjectType = any, D extends ObjectType = any> = CFormProps<
  T,
  D,
  DefaultBuiltInComponentMapType
>;
...
export interface CFormBaseConfig<T extends ObjectType = any, D extends ObjectType = any> {
   ...
   * 默认锚点 1 级
   */
  anchor?: boolean | { isOn: boolean; exclude?: FormPathPattern[]; level?: number };
  /**
   * 开启编辑后退出表单二次确认
   * @default false
   * @title 自定义 title
  ...
  unMountCFormSecondCheck?:
    | boolean
    | { title?: string; content?: string; excludes?: string[]; customEqual?: (form: CForm<T, D>) => boolean };
  /**
...
export interface CFormConfig<
  T extends ObjectType = any,
  D extends ObjectType = any,
  ComponentsMap extends PureComponentsMap = DefaultBuiltInComponentMapType,
> extends CFormBaseConfig<T, D> {
  /**
   * @zh 字段配置
   */
...
export type DefaultCFormConfig<T extends ObjectType = any, D extends ObjectType = any> = CFormConfig<
  T,
  D,
  DefaultBuiltInComponentMapType
>;
 ...
 */
export interface CFormStepConfig<
  T extends ObjectType = any,
  D extends ObjectType = any,
  ComponentsMap extends PureComponentsMap = any,
> extends CFormBaseConfig<T, D> {
  /**
   * @zh 是否展示步骤条
   */
...
export type DefaultCFormStepConfig<T extends ObjectType = any, D extends ObjectType = any> = CFormStepConfig<
  T,
  D,
  DefaultBuiltInComponentMapType
>;
 ...
 */
export interface CFormStepUnitConfig<
  T extends ObjectType = any,
  D extends ObjectType = any,
  ComponentsMap extends PureComponentsMap = DefaultBuiltInComponentMapType,
> {
...
export type DefaultCFormStepUnitConfig<T extends ObjectType = any, D extends ObjectType = any> = CFormStepUnitConfig<
  T,
  D,
  DefaultBuiltInComponentMapType
>;
...
export type BasicFieldType<
  ...
  D extends ObjectType = any,
  ComponentsMap extends PureComponentsMap = DefaultBuiltInComponentMapType,
> = { type?: 'Field' | 'ArrayField' | 'ObjectField' } & CFieldProps<T, D, ComponentsMap>;

export type CFieldConfig<
  T extends ObjectType = any,
  D extends ObjectType = any,
  ComponentsMap extends PureComponentsMap = DefaultBuiltInComponentMapType,
> = VoidFieldType<T, D, ComponentsMap> | BasicFieldType<T, D, ComponentsMap>;

export type DefaultCFieldConfig<T extends ObjectType = any, D extends ObjectType = any> = CFieldConfig<
  T,
  D,
  DefaultBuiltInComponentMapType
>;
...
export interface CFieldProps<
  ...
  D extends ObjectType = any,
  ComponentsMap extends PureComponentsMap = DefaultBuiltInComponentMapType,
> extends Omit<IFieldProps, BaseOmitKey | 'dataSource'>,
    CBaseFieldProps<T, D, ComponentsMap> {
  /**
...
export interface CVoidFieldProps<
  ...
  D extends ObjectType = any,
  ComponentsMap extends PureComponentsMap = DefaultBuiltInComponentMapType,
> extends Omit<IVoidFieldProps, BaseOmitKey>,
    CBaseFieldProps<T, D, ComponentsMap> {}

export interface CBaseFieldProps<
  ValueType extends ObjectType = any,
  DataType extends ObjectType = any,
  ComponentsMap extends PureComponentsMap = DefaultBuiltInComponentMapType,
> {
  /**
   * @zh 字段标题
 ...
 */
export type CFormHeader<T extends ObjectType = any, D extends ObjectType = any> =
  | CFormHeaderItem[]
  | ((form: CForm<T, D>) => CFormHeaderItem[]);
...
export type CFormHeaderBuiltInHeader = { title: string; onBack: () => void };
...
export type CFormStepHeaderBuiltInHeader = { title: string; onBack: () => void };
 ...
 */
export type CFormTop<T extends ObjectType = any, D extends ObjectType = any> =
  | CFormTopItem<T, D>[]
  | ((form: CForm<T, D>) => CFormTopItem[]);
export type CFormTopItem<T extends ObjectType = any, D extends ObjectType = any> =
  | { field: CFieldConfig<T, D> }
  | CFormTopCustomRender
  | CFormTopBuiltInAlert;
export type CFormTopCustomRender = (form: CForm) => ReactNode;
export type CFormTopBuiltInAlert = { alertType: 'info' | 'success' | 'warning' | 'error'; props?: AlertProps };

export type CFormStepTop<T extends ObjectType = any, D extends ObjectType = any> =
  | CFormStepTopItem<T, D>[]
  | ((formStep: CFormStep) => CFormStepTopItem[]);
export type CFormStepTopItem<T extends ObjectType = any, D extends ObjectType = any> =
  | { field: CFieldConfig<T, D> }
  | CFormStepTopCustomRender
  | CFormStepTopBuiltInAlert;
export type CFormStepTopCustomRender = (formStep: CFormStep) => ReactNode;
export type CFormStepTopBuiltInAlert = { alertType: 'info' | 'success' | 'warning' | 'error'; props?: AlertProps };
 ...
 */
export type CFormFooter<T extends ObjectType = any, D extends ObjectType = any> =
  | ((form: CForm<T, D>, onSubmit: (values: T) => void) => CFormFooterConfig<T, D>)
  | CFormFooterConfig<T, D>;
...
export type CFormFooterItem<T extends ObjectType = any, D extends ObjectType = any> =
  | { field: CFieldConfig<T, D> }
  | CFormFooterCustomRender
  | CFormFooterBuiltInButton;

export type CFormStepFooter<T extends ObjectType = any, D extends ObjectType = any> =
  | ((formStep: CFormStep, onSubmit: (values: T) => void) => CFormStepFooterConfig<T, D>)
  | CFormStepFooterConfig<T, D>;
 ...
 * @description
 */
export type CFormResidentContent<T extends ObjectType = any, D extends ObjectType = any> = CFormResidentContentItem<
  T,
  D
>[];
export type CFormResidentContentItem<T extends ObjectType = any, D extends ObjectType = any> =
  | { field: CFieldConfig<T, D> }
  | CFormResidentContentCustomRender
  | CFormResidentContentBuiltInButton;
export type CFormResidentContentBuiltInButton = { buttonType: 'confirm'; props?: ButtonProps };
export type CFormResidentContentCustomRender = (form: CForm) => ReactNode;

export type CFormStepResidentContent<
  T extends ObjectType = any,
  D extends ObjectType = any,
> = CFormStepResidentContentItem<T, D>[];
export type CFormStepResidentContentItem<T extends ObjectType = any, D extends ObjectType = any> =
  | { field: CFieldConfig<T, D> }
  | CFormStepResidentContentCustomRender
  | CFormResidentContentBuiltInButton;
 ...
 */
export interface RulesProps<T extends ObjectType = any, D extends ObjectType = any>
  extends Omit<IValidatorRules, 'triggerType' | 'validator'> {
  /**
   * @zh 触发校验的时机
...
export {
  ConfigPreviewComponentProps,
  ConfigPreviewItem,
  ConfigPreviewItemCustomRender,
  ConfigPreviewItemBuiltInRender,
  ConfigPreviewItemFormatterData,
} from './react/Components/ConfigPreview/index';

export { FeeCalculatorProps } from './react/Components/FeeCalculator/index';

export {
  CAsyncSelectDataSource,
  CAsyncSelectFormProps,
  CAsyncSelectFetchDataProps,
  CAsyncSelectInitDataSource,
} from './react/Components/CAsyncSelect/index';
...
export interface CFormRegisterConfigOptions<
  CM extends PureComponentsMap,
  D extends JSXComponent | keyof CM = JSXComponent | keyof CM,
> {
  ...
  //  {
  //   component: D;
  //   props?: D extends keyof CM ? React.ComponentProps<CM[D]> : D extends JSXComponent ? React.ComponentProps<D> : never;
  // };
  formLayout?: IFormLayoutProps;
}
...
export interface CFormInstanceProps<T extends ObjectType = any, D extends ObjectType = any>
  extends Omit<IFormProps<T>, 'effects'> {
  data?: D;
  /**
   * @zh 步骤表单默认展示第几步
   */
...
export type CallableType<
  ...
  ValueType extends ObjectType = any,
  DataType extends ObjectType = any,
  ParentType extends CGeneralField<ValueType, DataType> = any,
> = ReturnType | ((form: CForm<ValueType, DataType>, parentField: ParentType) => ReturnType);

export interface CVoidField<T extends ObjectType = any, D extends ObjectType = any> extends Omit<VoidField, 'form'> {
  form: CForm<T, D>;
}

export interface CField<T extends ObjectType = any, D extends ObjectType = any, V = any>
  extends Omit<Field<any, any, any, V>, 'form'> {
  form: CForm<T, D>;
}
...
export interface CObjectField<T extends ObjectType = any, D extends ObjectType = any>
  extends Omit<ObjectField, 'form'> {
  form: CForm<T, D>;
}

export type CGeneralField<T extends ObjectType = any, D extends ObjectType = any> =
  | CVoidField<T, D>
  | CField<T, D>
  | CArrayField<T, D>
  | CObjectField<T, D>;
...
export type ComponentOptions<
  T extends PureComponentsMap,
  ValueType extends ObjectType = any,
  DataType extends ObjectType = any,
> = (
  | ComponentMapToUnionType<T, [CForm, CGeneralField<ValueType, DataType>], 'props'>
  | {
 ...
 */
export interface CFormConfirmColumn<T extends ObjectType = any, D extends ObjectType = any>
  extends Omit<ItemProps, 'hidden' | 'label' | 'content'> {
  /**详情页展示字段名 */
  name?: string;
...
export interface CFormConfirmDetail<T extends ObjectType = any, D extends ObjectType = any> {
  ...
  /**详情页协议 */
  protocol?: CAgreementProps & { protocolLabel?: string };
  /**全局设置label宽度 */
  labelWidth?: number;
  /**确认页title，默认为”确认页“ */
  stepTitle?: string;
...
export interface ConfirmDetail<T extends ObjectType = any, F = any> {
  ...
  labelWidth?: number;
  /**详情页信息 */
  detail: (Omit<ConfirmStep, 'section'> & { section: ConfirmSection<ItemProps, T, F>[] })[];
  /**可编辑操作返回的步骤回调*/
