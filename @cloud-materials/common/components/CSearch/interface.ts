...
/** 组件内容 */
export type CSearchComponentContent =
  | (ReactElement & { component?: undefined })
...
export interface UseCCombineSearchValue {
  ...
  placeholder: string;
  /** 默认查询字段 */
  defaultField?: { fieldName: string; type: 'auto' | 'default' };
  /** 单选和多选时字段搜索词 */
  searchWord?: string;
  /** 校验错误信息 */
  errState?: boolean;
...
export interface UseCCombineSearchCustomProps extends PropsWithBase, PropsWithValue<CSearchParams> {
  ...
  listGroup?: { label: string; fieldNames: CCombineSearchItem['fieldName'][] }[];
  /** 缓存关键字，设置后参数会存储在 localStorage 中 */
  cacheKey?: string;
  /** 占位符 */
  placeholder?: string;
   ...
   */
  defaultField?: string | { fieldName: string; type?: 'auto' | 'default' };
  /** 加载态 */
