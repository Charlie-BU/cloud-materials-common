...
export type ListInLogProps = Partial<
  Pick<
    ListProps,
    | 'offsetBottom'
    | 'throttleDelay'
 ...
 * @title CLogProps
 */
export interface CLogProps
  extends Pick<ListProps, 'offsetBottom' | 'throttleDelay' | 'scrollLoading' | 'virtualListProps' | 'onReachBottom'> {
  style?: CSSProperties;
  ...
  /** 本地数据 */
  dataSource?: { total: number; data: LogDataItem[] };
  /** 获取日志数据 */
  fetcher?: (currentPage: number) => Promise<{ total: number; data: LogDataItem[] }>;
  /**
   * @zh 自定义渲染
   */
