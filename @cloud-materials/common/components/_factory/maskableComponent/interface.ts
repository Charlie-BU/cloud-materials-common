...
export type BuiltInFormProps<
  MaskableProps extends BaseProps,
  FormConfigKey extends FormConfigKeyUnion,
  ComponentsMap extends PureComponentsMap,
  FormData extends ObjectType = ObjectType,
  GlobalData extends ObjectType = ObjectType,
  OnOkReturn = any,
  OnCancelReturn = any,
> = Omit<MaskableProps, 'onOk' | 'onCancel'> & {
  footerFields?: CFormFooterItem<FormData, GlobalData>[];
  decoratorProps?: MaskableFormDecoratorProps;
  formFooter?: (
    ...args: FormConfigKey extends 'config'
      ? [renderCancelBtn: CFormFooterCustomRender, okBtn: CFormFooterBuiltInButton, form: CForm<FormData, GlobalData>]
      : [
...
export interface CreateBuiltIn<MaskableProps extends BaseProps, FormConfigKey extends FormConfigKeyUnion> {
  <ComponentsMap extends PureComponentsMap>(
    Form: React.JSXElementConstructor<CFormProps<any, any, ComponentsMap, any>>,
  ): (<FormData extends ObjectType = ObjectType, GlobalData extends ObjectType = ObjectType>(
    props: BuiltInFormProps<MaskableProps, FormConfigKey, ComponentsMap, FormData, GlobalData>,
  ) => React.ReactElement | null) & {
    create: CreateBuiltIn<MaskableProps, FormConfigKey>;
  } & Omit<
      ReturnType<
        CreateStaticMethods<React.JSXElementConstructor<BuiltInFormProps<MaskableProps, FormConfigKey, ComponentsMap>>>
      >,
      'open'
    > & {
      open: <
      ...
      >(
        ...
        BuiltInFormProps<MaskableProps, FormConfigKey, ComponentsMap, FormData, GlobalData, OnOkReturn, OnCancelReturn>,
        OnOkReturn | OnCancelReturn
      >;
    };
}

export interface CreateBuiltInForm {
  <MaskableProps extends BaseProps, FormConfigKey extends FormConfigKeyUnion>(
    Component: React.ComponentType<MaskableProps>,
    formType: FormConfigKey,
    options: {
      plugin?: new () => Plugin<MaskableProps>;
      defaultProps?: MaskableProps;
      /** 组件名 */
      componentName: Extract<
        keyof CComponentConfig,
        'CModal.Form' | 'CModal.FormStep' | 'CDrawer.Form' | 'CDrawer.FormStep'
      >;
    },
  ): CreateBuiltIn<MaskableProps, FormConfigKey>;
}

export interface CreateStaticMethods<
  T extends React.JSXElementConstructor<CommonComponentTypeProps> = React.JSXElementConstructor<CommonComponentTypeProps>,
> {
  <C extends T>(Component: C): C & {
    /** 通过js函数打开一个组件 */
    open: <P extends any = any>(
      props: StaticMethodsExtraProps<PickReactComponentProps<C>>,
    ) => StaticMethodsReturn<PickReactComponentProps<C>, P>;
...
export interface CreateMaskableComponent {
  <
    C extends React.JSXElementConstructor<BaseProps>,
    M extends Record<
      string,
      React.JSXElementConstructor<any> & ReturnType<CreateStaticMethods<React.JSXElementConstructor<any>>>
    >,
  >(
    Component: C,
    attachComponentsMap: M,
  ): ReturnType<CreateStaticMethods<C>> &
    DefineStatic<M> & {
...
export interface Plugin<T extends BaseProps> {
  ...
  getFormProps: (
    oldFormProps: React.ComponentProps<FormType<any, typeof MaskableFormDecorator>>,
  ) => typeof oldFormProps;
}
