...
import type {
  ...
  TableConfig as TableCompConfig,
  TableProps as TableCompProps,
} from '../../CTable/arco/types';
...
export interface TableConfig<DataItemType extends R>
  extends Omit<TableCompConfig<DataItemType>, 'columns' | 'globalColumnConfig' | 'rowKey'> {
...
export type RowFieldConfig =
  | Partial<IFieldProps<JSXComponent, JSXComponent>>
  | ((initialValue: R, form: Form, tableEditor: TableEditorModel) => Partial<IFieldProps<JSXComponent, JSXComponent>>);
...
export interface BaseTableEditorConfig<DataItemType extends R = any, ValueType extends R = any> {
   ...
   */
  formConfig?:
    | FormConfig<ValueType>
    | ((tableEditor: TableEditorModel<DataItemType, ValueType>) => FormConfig<ValueType>);
  /**
...
export interface UncontrolledTableEditorConfig<DataItemType extends R = any, ValueType extends R = any>
  extends BaseTableEditorConfig<DataItemType, ValueType> {
  /** 是否受控 */
  controlled?: false;
}

/** 受控 TableEditor 的 config */
export interface ControlledTableEditorConfig<DataItemType extends R = any, ValueType extends R = any>
  extends BaseTableEditorConfig<DataItemType, ValueType> {
  /** 是否受控 */
  controlled: true;
...
export type TableEditorConfig<DataItemType extends R = any, ValueType extends R = any> =
  | ControlledTableEditorConfig<DataItemType, ValueType>
  | UncontrolledTableEditorConfig<DataItemType, ValueType>;

/** 受控组件的 props */
export type ControlledTableEditorProps<DataItemType extends R = any, ValueType extends R = any> = Omit<
  TableCompProps<DataItemType>,
  'config' | 'table'
> & {
...
/** 非受控组件的 props */
export type UncontrolledTableEditorProps<DataItemType extends R = any, ValueType extends R = any> = Omit<
  TableCompProps<DataItemType>,
  'config' | 'table'
> & {
...
export type TableEditorProps<DataItemType extends R = any, ValueType extends R = any> =
  | ControlledTableEditorProps<DataItemType, ValueType>
  | UncontrolledTableEditorProps<DataItemType, ValueType>;
