...
import { transformArrayToFormValues, transformFormValuesToArray, addRowKeyToArray, addRowKey, removeRowKey, } from '../../utils';
// 先实现 MVP 版，再往里面加东西
...
var TableEditor = /** @class */ (function () {
    function TableEditor(options) {
        ...
        var tableConfig = isFunction(_tableConfig) ? _tableConfig(this) : _tableConfig;
        var formConfig = isFunction(_formConfig) ? _formConfig(this) : _formConfig;
        this.config = config;
        this.actionHistory = new ActionHistory();
        this.form = createForm(__assign(__assign({}, formConfig), { effects: function (form) {
                var _a;
                ...
                onFieldValueChange('*.*', function (field, form) {
                    ...
                    var oldValue = get(_this.previousFormValues, fieldPath);
                    _this.createAction({
                        undo: function () {
                            isUndoing = true;
                            field.setValue(oldValue);
                            // 将 previousFormValues 设置为当前值
                            set(_this.previousFormValues, fieldPath, oldValue);
                            isUndoing = false;
                        },
                    });
                    _this.previousFormValues = toJS(form.values);
                });
                // 2023-06-14 更新
                ...
                onFormValidateFailed(function (form) {
                    ...
                    if (firstErrorAdrress) {
                        var dom_1 = document.getElementById(getFormFieldId(firstErrorAdrress));
        ...
        this.table = createTable(__assign(__assign({}, tableConfig), { rowKey: function (record) { return record[ROW_KEY]; }, 
            // 给数据加 rowKey
            data: tableConfig.data ? addRowKeyToArray(tableConfig.data) : undefined, fetcher: tableConfig.fetcher
                ? function (options) {
                    return tableConfig.fetcher(options).then(function (res) { return (__assign(__assign({}, res), { data: addRowKeyToArray(res.data) })); });
                }
                : undefined, 
            ...
            effects: function (options) {
                ...
                onTableUpdateDataEnd(function (_a) {
                    ...
                    if (isFullUpdate) {
                        ...
                        if (!_this.form)
                            return;
                        var currentValues = toJS(_this.form.values);
                        // 2023-09-21更新，改为读取 initTotalData，因为 initTotalData 才是 table 的全量数据，totalData 只是当前数据（比如受控模式下筛选了就会改变）
                        var newValues = transformArrayToFormValues(table.initTotalData);
                        if (!isEqual(currentValues, newValues)) {
                            ...
                            var rowKeysToRemove = difference(currentRowKeys, newRowKeys);
    ...
    TableEditor.prototype.clearFormFields = function (rowKeys) {
        ...
        rowKeys.forEach(function (rowKey) {
            ...
            if (rowField) {
                fieldsToClear = __spreadArray(__spreadArray([], __read(fieldsToClear), false), [rowField], false);
            }
            if (cellField.length) {
                fieldsToClear = __spreadArray(__spreadArray([], __read(fieldsToClear), false), __read(cellField), false);
            }
        });
    ...
    TableEditor.prototype.innerAddRows = function (dataList, options) {
        var _this = this;
        var dataWithRowKey = dataList.map(function (d) { return (__assign(__assign({}, d), { data: addRowKey(d.data) })); });
        // 从 table 中增加这一行的结构
        ...
        var cloneDeepData = cloneDeep(dataWithRowKey);
        cloneDeepData.forEach(function (_a, index) {
            var data = _a.data;
            if (index === dataWithRowKey.length - 1) {
                _this.isFormValueBatchChanging = false;
            }
            _this.form.setValuesIn(data[ROW_KEY], data);
        });
    };
    Object.defineProperty(TableEditor.prototype, "addedRows", {
        ...
        get: function () {
            var _this = this;
            var formValues = toJS(this.form.values);
            // 2023-09-21更新，改为读取 initTotalData，因为 initTotalData 才是 table 的全量数据，totalData 只是当前数据（比如受控模式下筛选了就会改变）
            var tableData = this.table.initTotalData;
            return tableData.reduce(function (acc, curr) {
                var rowKey = curr[ROW_KEY];
                // todo: 后续优化为 map，降低复杂度
                if (!_this.originalTableData.find(function (v) { return v[ROW_KEY] === rowKey; })) {
                    acc.push({
                        key: rowKey,
                        value: removeRowKey(__assign(__assign({}, curr), formValues[rowKey])),
                    });
                }
                return acc;
            }, []);
    ...
    Object.defineProperty(TableEditor.prototype, "deletedRows", {
        ...
        get: function () {
            ...
            return this.originalTableData.reduce(function (acc, curr) {
                var rowValue = formValues[curr[ROW_KEY]];
                if (!rowValue) {
                    acc.push({
                        key: curr[ROW_KEY],
    ...
    Object.defineProperty(TableEditor.prototype, "updatedRows", {
        ...
        get: function () {
            ...
            return this.originalTableData.reduce(function (acc, curr) {
                ...
                var oldValue = removeRowKey(curr);
                var newValue = removeRowKey(__assign(__assign({}, oldValue), rowValue));
    ...
    Object.defineProperty(TableEditor.prototype, "hasChanged", {
        ...
        get: function () {
            var oldValue = this.originalTableData.map(removeRowKey);
            var newValue = this.makeTableDataSnapShot().map(removeRowKey);
    ...
    TableEditor.prototype.addRow = function (data, options) {
        var _this = this;
        var dataWithRowKey = addRowKey(data);
        var rowKey = dataWithRowKey[ROW_KEY];
        this.createAction({
            immediately: true,
            execute: function () {
    ...
    TableEditor.prototype.addRows = function (dataList, options) {
        var _this = this;
        var dataListWithRowKey = addRowKeyToArray(dataList);
        var rowKeys = dataListWithRowKey.map(function (d) { return d[ROW_KEY]; });
        this.createAction({
            immediately: true,
            execute: function () {
    ...
    TableEditor.prototype.deleteRow = function (rowKey) {
        ...
        var rowData = __assign(__assign({}, toJS(row.data)), toJS(this.form.values[rowKey]));
        var deletedData = {
            data: rowData,
    ...
    TableEditor.prototype.deleteRows = function (rowKeys) {
        ...
        var deletedData = _rowKeys.map(function (rowKey) {
            ...
            // 需要同时使用 table row 和 form field 的数据
            var rowData = __assign(__assign({}, toJS(row.data)), toJS(_this.form.values[rowKey]));
            return {
                data: rowData,
                option: { index: row.index },
            };
