...
var mapFilterConfig = function (column, arcoPrefixCls) {
    ...
    var filterComponentProps = getFnComponentProps(column.filterComponentProps, {
        table: column.table,
        column: column,
    });
    ...
    if (column.filterIconComponentType) {
        ...
        if (Component) {
            config.filterIcon = React.createElement(Component, __assign({}, defaultComponentProps, column.filterIconComponentProps));
        }
    }
    if (column.filterComponentType) {
        config.filterDropdown = function (options) {
            return (React.createElement("div", { className: "".concat(arcoPrefixCls, "-table-custom-filter") },
                React.createElement(FilterDropdown, { dataIndex: column.config.dataIndex, dropdownOptions: options })));
        };
    }
...
var mapColumnConfig = function (table, column, ctx) {
    ...
    var mergedColumnProps = __assign(__assign({}, columnProps), (isFn(column.config.arcoColumnProps)
...
var autoScrollX = function (table) {
    ...
    var scrollX = 0;
    try {
        for (var _c = __values(table.columns.filter(function (c) { return c.show; })), _d = _c.next(); !_d.done; _d = _c.next()) {
            var c = _d.value;
...
export var mapTableConfig = function (table, ctx) {
    var columns = table.columns
        .filter(function (c) { return c.show; })
        .map(function (columnModel) { return mapColumnConfig(table, columnModel, ctx); });
    var extraConfig = table.config.extraConfig;
    var isRefresh = table.loading && table.status.statusChangeReason === StatusChangeReason.REFRESH;
    ...
    var emptyHeight = 182;
    var noDataElement = React.createElement("div", { style: { height: emptyHeight } });
    if (!table.loading && table.rows.length === 0) {
        ...
        if (table.status.error && !table.status.error.__CANCEL__) {
            noDataElement = (React.createElement(CLoadingV2, { style: { height: emptyHeight }, type: "block", hasError: true, onReload: function () { return table.refresh(); } }));
        }
        else {
            ...
            untracked(function () {
                ...
                if (isReactNode(_noDataElement)) {
                    ...
                    noDataElement = (React.createElement(CLoadingV2.Result, __assign({ style: { height: emptyHeight }, status: React.createElement(IconNoDataHighSaturation, null), title: ctx.locale.CTable.noData }, _noDataElement)));
                }
            });
        }
    }
    var tableProps = {
        ...
        rowClassName: function (record) {
            ...
            return ((_a = table.initStatus) === null || _a === void 0 ? void 0 : _a.activeRowKey) === record.key ? "".concat(prefixCls, "-active-row") : '';
        },
        pagination: hidePagination
            ? false
            : __assign({ showTotal: true, total: table.total, pageSize: table.pageSize, 
                ...
                }, sizeOptions: [10, 20, 50, 100] }, (isPlainObj(table.config.pagination) ? table.config.pagination : {})),
        onChange: function (_, sorter, filters, extra) {
            ...
            if (extra.action === 'sort') {
                ...
                var sorterInfo = isArray(sorter) ? sorter[0] : sorter;
        ...
        if (rowSelection_1.component) {
            tableProps.rowSelection = {
                renderCell: function (_, __, record) {
                    return React.createElement(RowSelection, { record: record });
                },
            };
        }
        else {
            tableProps.rowSelection = {
                ...
                checkboxProps: function (record) {
                    ...
                    var rowSelectionComponentProps = getFnComponentProps(row.rowSelectionComponentProps, { table: row.table, row: row, rowData: row.data });
                    return __assign(__assign({}, rowSelectionComponentProps), { disabled: ((_a = rowStatusMap[row.key]) === null || _a === void 0 ? void 0 : _a.selectable) === false });
                },
            };
        }
    }
    if (table.config.expandRow) {
        ...
        tableProps.expandedRowRender = function (record) { return React.createElement(ExpandRow, { record: record }); };
        ...
        tableProps.expandProps = {
            ...
            icon: function (_a) {
                var expanded = _a.expanded, record = _a.record, restProps = __rest(_a, ["expanded", "record"]);
                return expanded ? (React.createElement("button", __assign({}, restProps, { type: "button" }),
                    React.createElement(IconMinus, null))) : (React.createElement("button", __assign({}, restProps, { type: "button" }),
    ...
    if (table.sumRowData) {
        tableProps.summary = function () {
            return (React.createElement(Table.Summary, null,
                React.createElement(Table.Summary.Row, null, table.columns
                    .filter(function (c) { return c.show; })
                    .map(function (c) {
                    var _a;
                    return (React.createElement(Table.Summary.Cell, { key: c.config.dataIndex }, (_a = getCellData(table.sumRowData, c.config.dataIndex)) !== null && _a !== void 0 ? _a : '-'));
                }))));
        };
    }
    return tableProps;
