 ...
 * @param table
 */
export var hidePaginationWhenTotalLessThan = function (table) {
    var _a;
...
export var shouldHideBottomButton = function (table) {
...
export var getArcoPropsFilterPagination = function (table, 
...
mergedArcoProps) {
    ...
    if (((_a = table.config.extraConfig) === null || _a === void 0 ? void 0 : _a.useArcoPagination) === true) {
        return __assign({}, mergedArcoProps);
    }
    // 渲染自定义分页器时，把 arco table 的 pagination 干掉
    return __assign(__assign({}, mergedArcoProps), { pagination: false });
};
// 处理自定义分页的渲染
export var controlPagination = function (table, mergedArcoProps, tablePrefixCls, arcoPrefixCls) {
    var _a, _b;
    /**
    ...
    // 原始的分页配置
    var paginationNode = pagination && React.createElement(Pagination, __assign({}, pagination));
    ...
    if (renderPagination) {
        return React.createElement("div", { className: containerClassName }, renderPagination(paginationNode));
    }
    var showPagination = Boolean(pagination);
    var showBottomLeft = Boolean((_b = table.config.toolbar) === null || _b === void 0 ? void 0 : _b.bottomLeft) && !shouldHideBottomButton(table);
    if (showPagination || showBottomLeft) {
        return (React.createElement("div", { className: cls(containerClassName, showBottomLeft && "".concat(tablePrefixCls, "-pagination-with-toolbar"), showBottomLeft && table.selectedRowKeys.length > 0 && "".concat(tablePrefixCls, "-pagination-with-toolbar-highlight")) },
            showBottomLeft && React.createElement(BottomLeftToolbar, { config: table.config.toolbar }),
