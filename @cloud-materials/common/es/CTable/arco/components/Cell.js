...
var CellComponent = observer(function (_a) {
    ...
    var modelProps = __assign(__assign({}, options), { content: content, cellData: cell.data });
    if (isFn(componentProps)) {
        componentProps = componentProps(modelProps);
    }
    var _b = getCellDataCy(cell, prefix), dataCy = _b.dataCy, dataCyIdx = _b.dataCyIdx;
    /*
  ...
  */
    ...
    if (column.config.render) {
        ...
        cellContent = Component(__assign(__assign(__assign({}, defaultComponentProps), (cell.component in CellComponentsEnum ? omitCamelProps(modelProps) : modelProps)), componentProps));
    }
        ...
        cellContent = React.createElement(Comp, __assign({}, defaultComponentProps, modelProps, componentProps));
    }
         ...
         */
        var Comp = Component;
        cellContent = (React.createElement(Comp, __assign({}, defaultComponentProps, (cell.component in CellComponentsEnum ? omitCamelProps(modelProps) : modelProps), componentProps)));
    }
    else {
    ...
    // 这里和 arco 保持一致
    React.createElement("span", { className: arcoCellSpanClassname, "data-testid": dataCy, "data-cy": dataCy, "data-cy-idx": dataCyIdx, style: { marginRight: column.config.alignType === 'right' ? getAlignRightWidth(column) : undefined } }, fallbackText(cellContent)));
});
export var TableCell = observer(function (_a) {
    var column = _a.column, className = _a.className;
    return (React.createElement(CellProvider, { dataIndex: column.dataIndex }, function (options) {
        return renderDecorator(options.table, React.createElement(CellComponent, { options: options, arcoCellSpanClassname: className }), {
            scope: 'cell',
...
export var TableCellMemo = memo(TableCell, function (prev, next) {
    /**
