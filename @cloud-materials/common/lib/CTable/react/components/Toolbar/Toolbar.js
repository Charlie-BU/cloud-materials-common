"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.ToolbarProvider = exports.ToolbarContext = void 0;
var tslib_1 = require("tslib");
/*
 * @Author: youjingyu
 * @Date: 2021-09-26 12:14:11
 * @LastEditTime: 2021-10-13 17:37:54
 * @LastEditors: youjingyu
 * @Description:
 */
var react_1 = tslib_1.__importStar(require("react"));
var react_2 = require("@formily/react");
var hooks_1 = require("../../hooks");
var shared_1 = require("../../../shared");
exports.ToolbarContext = (0, react_1.createContext)(null);
exports.ToolbarProvider = (0, react_2.observer)(function (_a) {
    var children = _a.children;
    var table = (0, hooks_1.useTable)();
    var toolbar = table.toolbar;
    var options = { table: table, toolbar: toolbar };
    var childrenNode = (0, shared_1.isFn)(children) ? children(options) : children;
    return react_1.default.createElement(exports.ToolbarContext.Provider, { value: toolbar }, childrenNode);
});
//# sourceMappingURL=Toolbar.js.map